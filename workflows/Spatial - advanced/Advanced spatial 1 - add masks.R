...
###################################################################################        
    ...
    ### Read in ROI channel TIFFs
        ...
        for(i in rois){
          spatial.dat[[i]] <- read.spatial.files(i)
        }
...
###################################################################################
    ...
    ### Import CELL TYPE masks
        ...
        cell.types <- list.files(pattern = cell.type.ext)
        as.matrix(cell.types)
        
        spatial.dat <- do.add.masks(dat = spatial.dat,
                                    mask.dir = MaskDirectory,
                                    mask.pattern = cell.type.ext,
    ...
    ### Import REGION masks
        ...
        regions <- list.files(pattern = region.ext)
        as.matrix(regions)
        
        spatial.dat <- do.add.masks(dat = spatial.dat,
                                    mask.dir = MaskDirectory,
                                    mask.pattern = region.ext,
                                    mask.label = 'region')
...
###################################################################################
    ...
    ### Generate polygons and outlines
        ...
        spatial.dat <- do.create.outlines(dat = spatial.dat, mask.name = 'region')
...
###################################################################################       
    ...
    factor.plot <- names(spatial.dat[[1]]@DATA$CellData)[20:21]
    factor.plot
