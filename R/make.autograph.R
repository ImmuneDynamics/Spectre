...
#' @param path DEFAULT = NULL. The location to save plots. By default, nothing will be saved. 
#' Set this a directory in your computer if you want to save the graph.
#'
...
make.autograph <- function(dat, 
    ...
    ### Require packages ggplot2, data.table, ggpubr, scales,
    ...
    # spectral.list <- colorRampPalette(brewer.pal(11,'Spectral'))(50) spectral.list <- rev(spectral.list) colour.scheme <-
    ...
    # max_y_value_p10 <- max_y_value*y.first.level max_y_value_p40 <- max_y_value*y.second.level
    ...
    p <- ggplot2::ggplot(dat, ggplot2::aes(x = Xaxis, y = dat[[y.axis]]))  

    ## PAIRED if(!is.null(pairing)){ p <- p <- p + ggplot2::geom_line(aes(group=pairing)) }

    ## VIOLIN
    if (violin == TRUE) {
        message("AutoGraph - adding violin plot")
        p <- p + ggplot2::geom_violin(ggplot2::aes(fill = as.factor(dat[[colour.by]]), 
                                                   colour = as.factor(dat[[colour.by]])), 
                             trim = FALSE, show.legend = FALSE, alpha = 0.1)
    }
    ...
    p <- p + ggplot2::geom_point(ggplot2::aes(fill = as.factor(dat[[colour.by]]), 
                                     colour = as.factor(dat[[colour.by]])), 
                                 shape = 21,
                                 colour = "Black", stroke = 1, alpha = 0.9, 
                                 size = dot.size, position = ggplot2::position_jitter(width = 0.1, height = 0))
    ...
    p <- p + ggplot2::labs(x = paste0(x.axis), y = y.axis.label, subtitle = subtitle)  # colnames(data)[3] would return the name -- use similar for loop -- maybe data$dose

    # OTHER OPTIONS scale_fill_brewer(palette='Dark2') + scale_color_brewer(palette='Dark2') +

    ## PRISM -- SE (from https://stackoverflow.com/questions/2676554/in-r-how-to-find-the-standard-error-of-the-mean)

    p <- p + ggplot2::stat_summary(fun.max = function(i) mean(i) + sd(i)/sqrt(length(i)), 
                                   fun.min = function(i) mean(i) - sd(i)/sqrt(length(i)),
        geom = "errorbar", width = 0.5, size = 1)

    p <- p + ggplot2::stat_summary(fun = mean, fun.min = mean, fun.max = mean, 
                                   geom = "crossbar", width = 0.7, size = 0.5)  # add a large point for the mean

    # VARIANCE stat_summary(fun.y=mean, geom='point', shape=18, size=8, color='Black') + # add a large point for the mean
    # geom_pointrange() + # need ymin and ymax geom_errorbar(aes(ymax = Sepal.Length+se, ymax = Sepal.Length-se)) + # SEM
    # -- requires prior calculation of SE geom_errorbar(aes(ymax = Sepal.Length+sd, ymax = Sepal.Length-sd)) +
    ...
    # MORE THAN TWO GROUPS: compare against reference sample stat_compare_means(method = 'kruskal.test', label.y = 45) + #
    # Add global p-value stat_compare_means(label = 'p.signif', method = 't.test',ref.group = '0.5', label.y = 40)

    ## VISUALS labs(title=paste0(i, ' ', plotname), x= paste0(X_axis_label), y = paste0(Y_axis_label)) + #
    ## colnames(data)[3] would return the name -- use similar for loop -- maybe data$dose
    ...
    ## SCALES scale_x_discrete(limits=Group_Order) + # use to re-arrange X axis values
    if (scale == "lin") {
    ...
    # if(scale == 'log2'){ p <- p + scale_y_continuous(trans = 'log2') } if(scale == 'log10'){ p <- p +
    # scale_y_continuous(trans = 'log10') # p <- p + scale_y_log10() }
