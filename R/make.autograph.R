...
make.autograph <- function(dat,
  ...
  if (violin == TRUE) {
    ...
    p <- p + geom_violin(ggplot2::aes(
      fill = as.factor(dat[[colour.by]]),
  ...
  p <- p + ggplot2::stat_summary(
    ...
    width = 0.7,
    size = 0.5
  ) # add a large point for the mean
  ...
  ## THEMES
  p <- p + ggplot2::theme(
    legend.position = "right", # can be "left" "right" "top" "bottom" "none
