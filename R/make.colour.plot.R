...
make.colour.plot <- function(dat,
  ...
  ### Load packages
  # require
  # ggplot2, scales, colorRamps, ggthemes, RColorBrewer
  ...
  if (colours == "viridis") {
    colour.scheme <-
      colorRampPalette(c(viridis::viridis_pal(option = "viridis")(50)))
  }
  
  # Inferno
  if (colours == "inferno") {
    colour.scheme <-
      colorRampPalette(c(viridis::viridis_pal(option = "inferno")(50)))
  }
  
  # Magma
  if (colours == "magma") {
    colour.scheme <-
      colorRampPalette(c(viridis::viridis_pal(option = "magma")(50)))
  }
  ...
  if (colours == "turbo") {
    colour.scheme <-
      colorRampPalette(c(viridis::viridis_pal(option = "turbo")(50)))
  }
  
  # Mako
  if (colours == "mako") {
    colour.scheme <-
      colorRampPalette(c(viridis::viridis_pal(option = "mako")(50)))
  }
  
  # Rocket
  if (colours == "rocket") {
    colour.scheme <-
      colorRampPalette(c(viridis::viridis_pal(option = "rocket")(50)))
  }
  ...
  if (!is.null(col.axis)) {
    if (col.type == "continuous") {
      if (is.null(align.col.by)) {
        ...
        ColrMin <-
          quantile(align.col.by[[col.axis]],
                   probs = c(col.min.threshold),
                   na.rm = TRUE)
        ColrMax <-
          quantile(align.col.by[[col.axis]],
  ...
  if (!is.null(col.axis)) {
    if (col.type == "continuous") {
      ...
      if ("data.table" %in% class(colours)) {
        ...
        col.vc <-
          setNames(as.character(colours[[1]]), colours[[2]])
        col.vc
  ...
  if (is.null(col.axis)) {
    ...
    if (colours == "viridis" ||
      ...
      p <-
        p + ggplot2::scale_colour_gradientn(colours = rev(colorRampPalette(
  ...
  if (col.type == "factor") {
    if (add.label == TRUE) {
      ...
      if (is.numeric(dat[[col.axis]])) {
        ...
        centroidY <-
          tapply(dat[[y.axis]], dat[[col.axis]], median)
        centroidCol <-
          tapply(dat[[col.axis]], dat[[col.axis]], median)
        
        centroidsDf <-
          data.frame(centroidX, centroidY, centroidCol)
      }
...
fast.colour.plot <- function(dat,
  ...
  colour.scheme <- get_colours(colours)
  ...
  if (is.null(col.axis)) {
    ...
    if (is_viridis_colour(colours)) {
      p <- p + viridis::scale_colour_viridis(option = colours)
    } else if (colours == "jet") {
      p <- p + scale_colour_gradientn(colours = get_jet_colour())
    } else if (is_rcolorbrewer_colour(colours)) {
      p <- p + scale_colour_gradientn(colours = get_rcolorbrewer_colour(colours))
    }
  }
  ...
  if (legend.loc %in% c("top", "bottom")) {
    ...
    # either left or right or garbled
...
get_colours <- function(colours) {
  if (colours == "jet") {
    return(colorRampPalette(get_jet_colour()))
  } 
  else if (is_viridis_colour(colours)) {
    return(get_viridis_palette(colours))
  }
  else if (is_rcolorbrewer_colour(colours)) {
    return(colorRampPalette(get_rcolorbrewer_colour(colour)))
  }
  else {
    stop(paste(colours, "neither is jet nor is available in either viridis or Rcolorbrewer packages."))
  }
}

is_viridis_colour <- function(colour) {
  VIRIDIS_COLOURS <- c(
    "viridis", "magma", "inferno", "plasma", "cividis", "rocket", "mako", "turbo"
  )
  return(colour %in% VIRIDIS_COLOURS)
}

is_rcolorbrewer_colour <- function(colour) {
  RCOLORBREWER_COLOURS <- c(
    "RdYlGn", "RdYlBu", "RdGy", "RdBu",
    "PuOr", "PRGn", "PiYG", "BrBg",
    "spectral", "BuPu"
  )
  return(colour %in% RCOLORBREWER_COLOURS)
}

get_rcolorbrewer_colour <- function(colour) {
  if (colour == "spectral") {
    colour.list <- colorRampPalette(RColorBrewer::brewer.pal(11, "Spectral"))(50)
  } else {
    colour.list <- colorRampPalette(RColorBrewer::brewer.pal(9, colour))(31)
  }
  colour.list <- rev(colour.list)
  return(colour.list)
}

get_viridis_palette <- function(colour) {
  return(
    colorRampPalette(c(viridis::viridis_pal(option = colour)(50)))
  )
}

get_jet_colour <- function() {
  return(c(
      "#00007F",
      "blue",
      "#007FFF",
...
can_plot_hex <- function(col.axis, dat) {
  if (is.null(col.axis)) {
    stop(
      "Note: hex bins do not currently work for density plots, only for colour plots when col.axis is specified and can be plotted as a continuous numeric variable"
    )
  } else if (!is.numeric(dat[[col.axis]])) {
    # tests to see if there are any non numeric values
    stop(
      "Sorry, hex bins only work when col.type is specified, and can be plotted as a continuous numeric variable"
    )
