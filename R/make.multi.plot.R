...
#'   dat = as.data.table(Spectre::demo.umap),
#'   x.axis = "UMAP_42_X",
#'   y.axis = "UMAP_42_Y",
...
make.multi.plot <- function(dat,
  ...
  if (!is.null(divide.by)) { # divide dataset by sample/group etc
    ...
    for (i in to.divide) {
      dat.list[[i]] <- dat[dat[[divide.by]] == i, ]
    }
  }
  ...
  for (a in names(dat.list)) {
    ...
    for (i in to.plot) {
      ...
      plots[[plot.nme]] <- make.colour.plot(
        dat = plot.dat, # instead, use d[d[$Sample][plot.by] == to.plot[i], ] ## Spectre::
        x.axis = x.axis,
        y.axis = y.axis,
        col.axis = i,
        col.type = col.type,
    ...
    if (length(dat.list) == 1) {
      if (add.density == TRUE) {
        ...
        plots[[length(plots) + 1]] <- make.colour.plot(
          dat = plot.dat,
          x.axis = x.axis,
          y.axis = y.axis,
          colours = colours,
  ...
  ggplot2::ggsave(
    filename = paste0(figure.title, " - plotted on ", x.axis, " by ", y.axis, ".png"),
    plot = gp,
    path = path,
    width = wdth,
    height = hght,
