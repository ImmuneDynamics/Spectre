...
test_that("add new data works", {
    ...
    expect_equal(test_obj@metadata$cyto_test$description, "test")
...
test_that("add new data with no metadata works", {
    ...
    expect_equal(ncol(test_obj$cyto_test), 2)
    expect_equal(nrow(test_obj$cyto_test), 10)
    expect_true("cyto_test" %in% names(test_obj@metadata))
...
test_that("add new metadata data works", {
    ...
    test_obj = add.new.metadata(
        spectre_obj = test_obj,
        metadata = c("test vector"),
        metadata_name = "test_descp",
        dataset_name = "cyto_test"
    )
    
    expect_true("test_descp" %in% names(test_obj@metadata$cyto_test))
    expect_equal(test_obj@metadata$cyto_test$test_descp, c("test vector"))
